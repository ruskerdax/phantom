...
function encExplodeMissile(enc, m, isEnemy){
  ...
  if(!isEnemy){
    for(const e of enc.en){if(!e.alive)continue;
      if(encDist(enc,m.x,m.y,e.x,e.y)<r+OET[e.t].enc.r){
...
function updEncMissiles(enc, mis, isEnemy, ew, eh){
  ...
  for(let i=mis.length-1;i>=0;i--){
    ...
    if(!det){
      if(!isEnemy){
        for(const e of enc.en){if(!e.alive)continue;
          if(encDist(enc,m.x,m.y,e.x,e.y)<OET[e.t].enc.r){
...
function updEnc(){
  ...
  for(let ri=enc.rocks.length-1;ri>=0;ri--){const rk=enc.rocks[ri];if(encDist(enc,s.x,s.y,rk.x,rk.y)<rk.r+7){
    ...
    if(dmg>0){const hit=applyShipDamage(s,dmg,{source:encPointNear(enc,rk.x,rk.y,s.x,s.y),kind:'collision'});shipDamageTone(hit,180,.15,'sawtooth',.12);}
...
function drawEnc(){
  ...
  for(const e of enc.en)if(e.alive)drawAt(e.x,e.y,(OET[e.t].enc.r||12)+16,(x,y)=>OET[e.t].drawEnc({...e,x,y}));
  if(enc.isHBase){
