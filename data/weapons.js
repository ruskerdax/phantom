...
const WEAPONS = [
  ...
  {id:'plasma blaster', name:'PLASMA BLASTER', wpnType:'energy', fireMode:'projectile', dmg:20, cd:0.1, spd:10, life:60, energyCost:3, aiPolicy:'tap', buyable:true, licensePrice:2000, buildPrice:300, drawProjectile:(b)=>{cx.save();cx.fillStyle='#0ff';cx.shadowColor='#0ff';cx.shadowBlur=sb(12);cx.beginPath();cx.arc(b.x,b.y,3.5,0,Math.PI*2);cx.fill();cx.globalAlpha=.65;cx.fillStyle='#fff';cx.beginPath();cx.arc(b.x,b.y,1.5,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'ion blaster', name:'ION BLASTER', wpnType:'energy', fireMode:'persistent-projectile', innerR:16, innerDmgPerTick:2, outerR:0, spd:4, life:80, cd:0.3, energyCost:3, aiPolicy:'tap', buyable:true, licensePrice:2200, buildPrice:320, drawProjectile:(p)=>{const fr=typeof G!=='undefined'?G.fr||0:0,pulse=.65+.35*Math.sin(fr*.28+(p.l||0)*.08);cx.save();cx.globalAlpha=.72;cx.fillStyle='#7df';cx.shadowColor='#7df';cx.shadowBlur=sb(14);cx.beginPath();cx.arc(p.x,p.y,4.5+pulse*2,0,Math.PI*2);cx.fill();cx.globalAlpha=.95;cx.fillStyle='#dff';cx.beginPath();cx.arc(p.x,p.y,2.2+pulse*.8,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'ion cannon', name:'ION CANNON', wpnType:'energy', fireMode:'charged-persistent-projectile', chargeMin:6, chargeMax:120, chargeEnergyDrainPerFrame:0.125, chargeHoldDrainPerFrame:0.04, innerRMin:12, innerRMax:36, outerRMin:30, outerRMax:90, innerDmgPerTick:1, outerDmgPerTick:1, outerTickInterval:4, spd:3, life:90, aiPolicy:null, buyable:true, licensePrice:5000, buildPrice:700, drawProjectile:(p)=>{const fr=typeof G!=='undefined'?G.fr||0:0,level=Math.max(0,Math.min(1,p.chargeLevel??0)),pulse=.65+.35*Math.sin(fr*.22+(p.l||0)*.1),r=4+level*7+pulse*2;cx.save();cx.globalAlpha=.45+.25*pulse;cx.fillStyle='#9df';cx.shadowColor='#9df';cx.shadowBlur=sb(16+level*18);cx.beginPath();cx.arc(p.x,p.y,r,0,Math.PI*2);cx.fill();cx.globalAlpha=.95;cx.fillStyle='#fff';cx.beginPath();cx.arc(p.x,p.y,2.2+level*2+pulse*.7,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'arc caster', name:'ARC CASTER', wpnType:'energy', fireMode:'charged-cone-chain', chargeMin:36, chargeMax:36, chargeEnergyDrainPerFrame:0, chargeHoldDrainPerFrame:0, fireEnergyCost:8, coneAngleRad:Math.PI/9, coneLength:320, primaryDmg:32, chainHopMaxDist:80, chainDamages:[15,8], chainHopsMax:2, cd:0.8, aiPolicy:null, buyable:true, licensePrice:5500, buildPrice:760, drawArc:(srcX,srcY,a,points)=>{if(!Array.isArray(points)||points.length===0)return;cx.save();cx.strokeStyle='#dff';cx.shadowColor='#9df';cx.shadowBlur=sb(14);cx.lineWidth=1.5;cx.beginPath();cx.moveTo(srcX,srcY);for(const p of points)cx.lineTo(p.x,p.y);cx.stroke();cx.strokeStyle='#fff';cx.lineWidth=.75;cx.shadowBlur=0;cx.beginPath();cx.moveTo(srcX,srcY);for(const p of points)cx.lineTo(p.x,p.y);cx.stroke();cx.restore();}},
  {id:'flak cannon', name:'FLAK CANNON', wpnType:'kinetic', fireMode:'magazine-burst', dmg:5, spd:12, life:30, burstCount:8, burstSpread:Math.PI*30/180, cd:0.15, magMax:5, ammoMax:100, reloadSec:3.0, aiPolicy:null, buyable:true, licensePrice:3200, buildPrice:420},
  {id:'rotary autocannon', name:'ROTARY AUTOCANNON', wpnType:'kinetic', fireMode:'spooled-projectile', dmg:5, spd:12, life:80, cd:7, spoolFrames:6, fireSpread:Math.PI*1/180, ammoMax:500, aiPolicy:null, buyable:true, licensePrice:3800, buildPrice:480, drawProjectile:(b)=>{cx.save();cx.fillStyle='#fff';cx.shadowColor='#fff';cx.shadowBlur=sb(5);cx.beginPath();cx.arc(b.x,b.y,2,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'shredder shot', name:'SHREDDER SHOT', wpnType:'kinetic', fireMode:'detonate-projectile', slugDmg:20, slugSpd:4, slugLife:90, pelletCount:8, pelletDmg:8, pelletSpd:12, pelletLife:30, pelletArcRad:Math.PI*25/180, ammoMax:40, cd:0.4, aiPolicy:null, buyable:true, licensePrice:3000, buildPrice:400, drawSlug:(b)=>{cx.save();cx.fillStyle='#bff';cx.shadowColor='#7ff';cx.shadowBlur=sb(12);cx.beginPath();cx.arc(b.x,b.y,6,0,Math.PI*2);cx.fill();cx.globalAlpha=.8;cx.fillStyle='#fff';cx.beginPath();cx.arc(b.x,b.y,2.4,0,Math.PI*2);cx.fill();cx.restore();}, drawPellet:(b)=>{cx.save();cx.fillStyle='#fff';cx.shadowColor='#fff';cx.shadowBlur=sb(4);cx.beginPath();cx.arc(b.x,b.y,1.8,0,Math.PI*2);cx.fill();cx.restore();}, drawProjectile(b){if(b.slug)this.drawSlug(b);else this.drawPellet(b);}},
