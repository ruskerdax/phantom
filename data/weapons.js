...
const WEAPONS = [
  {id:'mass driver',  name:'35mm Mass Driver',  wpnType:'kinetic', fireMode:'projectile', dmg:35, cd:1.0, spd:10,  life:100, ammoMax:200, aiPolicy:'tap', buyable:true, buildPrice: 200},
  {id:'mass driver small',  name:'18mm Mass Driver',  wpnType:'kinetic', fireMode:'projectile', dmg:18, cd:1.0, spd:10,  life:100, ammoMax:80, aiPolicy:'tap', buyable:false, licensePrice: 2200, buildPrice: 280},
  {id:'railgun',      name:'22mm Railgun',      wpnType:'kinetic', fireMode:'projectile', dmg:30, cd:2.0, spd:20, life:150, ammoMax:100, aiPolicy:'tap', buyable:true},
  {id:'pulse laser', name:'50 MJ Pulse Laser', wpnType:'energy', fireMode:'beam',    dmg:10, cd:1.5, range:267, pulses:5, pulseCd:5, persist:3, energyCost:3, aiPolicy:'beam-pulse', buyable:true, buildPrice: 250},
  {id:'pulse laser large', name:'80 MJ Pulse Laser', wpnType:'energy', fireMode:'beam',    dmg:20, cd:2.0, range:400, pulses:4, pulseCd:5, persist:3, energyCost:5, aiPolicy:'beam-pulse', buyable:true, licensePrice: 4500, buildPrice: 600},
  {id:'mining laser', name:'10 MJ Mining Laser', wpnType:'energy', fireMode:'beam', dmg:10, cd:1.0, range:150, pulses:1, pulseCd:5, persist:3, energyCost:1, aiPolicy:'beam-pulse', buyable:false},
  {id:'plasma blaster', name:'12 MK Plasma Blaster', wpnType:'energy', fireMode:'projectile', dmg:12, cd:0.15, spd:12, life:80, energyCost:1.5, aiPolicy:'tap', buyable:true, licensePrice:2000, buildPrice:300, drawProjectile:(b)=>{cx.save();cx.fillStyle='#0ff';cx.shadowColor='#0ff';cx.shadowBlur=sb(12);cx.beginPath();cx.arc(b.x,b.y,3.5,0,Math.PI*2);cx.fill();cx.globalAlpha=.65;cx.fillStyle='#fff';cx.beginPath();cx.arc(b.x,b.y,1.5,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'ion blaster', name:'38C Ion Disruptor', wpnType:'energy', fireMode:'persistent-projectile', innerR:16, innerDmgPerTick:3, outerR:0, spd:6, life:120, cd:0.3, energyCost:1.5, aiPolicy:'tap', buyable:true, licensePrice:2200, buildPrice:320, drawProjectile:(p)=>{const fr=typeof G!=='undefined'?G.fr||0:0,pulse=.65+.35*Math.sin(fr*.28+(p.l||0)*.08);cx.save();cx.globalAlpha=.72;cx.fillStyle='#7df';cx.shadowColor='#7df';cx.shadowBlur=sb(14);cx.beginPath();cx.arc(p.x,p.y,4.5+pulse*2,0,Math.PI*2);cx.fill();cx.globalAlpha=.95;cx.fillStyle='#dff';cx.beginPath();cx.arc(p.x,p.y,2.2+pulse*.8,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'ion cannon', name:'81C Ion Cannon', wpnType:'energy', fireMode:'charged-persistent-projectile', chargeMin:6, chargeMax:120, chargeEnergyDrainPerFrame:0.0625, chargeHoldDrainPerFrame:0.02, innerRMin:12, innerRMax:36, outerRMin:30, outerRMax:90, innerDmgPerTick:3, outerDmgPerTick:2, outerTickInterval:4, spd:5, life:120, aiPolicy:null, buyable:true, licensePrice:5000, buildPrice:700, drawProjectile:(p)=>{const fr=typeof G!=='undefined'?G.fr||0:0,level=Math.max(0,Math.min(1,p.chargeLevel??0)),pulse=.65+.35*Math.sin(fr*.22+(p.l||0)*.1),r=4+level*7+pulse*2;cx.save();cx.globalAlpha=.45+.25*pulse;cx.fillStyle='#9df';cx.shadowColor='#9df';cx.shadowBlur=sb(16+level*18);cx.beginPath();cx.arc(p.x,p.y,r,0,Math.PI*2);cx.fill();cx.globalAlpha=.95;cx.fillStyle='#fff';cx.beginPath();cx.arc(p.x,p.y,2.2+level*2+pulse*.7,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'arc caster', name:'1.5 MA Arc Projector', wpnType:'energy', fireMode:'charged-cone-chain', chargeMin:36, chargeMax:36, chargeEnergyDrainPerFrame:0, chargeHoldDrainPerFrame:0, fireEnergyCost:5, coneAngleRad:Math.PI/9, coneLength:320, primaryDmg:32, chainHopMaxDist:80, chainDamages:[15,8], chainHopsMax:2, cd:0.8, aiPolicy:null, buyable:true, licensePrice:5500, buildPrice:760, drawArc:(srcX,srcY,a,points)=>{if(!Array.isArray(points)||points.length===0)return;cx.save();cx.strokeStyle='#dff';cx.shadowColor='#9df';cx.shadowBlur=sb(14);cx.lineWidth=1.5;cx.beginPath();cx.moveTo(srcX,srcY);for(const p of points)cx.lineTo(p.x,p.y);cx.stroke();cx.strokeStyle='#fff';cx.lineWidth=.75;cx.shadowBlur=0;cx.beginPath();cx.moveTo(srcX,srcY);for(const p of points)cx.lineTo(p.x,p.y);cx.stroke();cx.restore();}},
  {id:'flak cannon', name:'55mm Flak Cannon', wpnType:'kinetic', fireMode:'magazine-burst', dmg:5, spd:15, life:30, burstCount:8, burstSpread:Math.PI*30/180, cd:0.15, magMax:5, ammoMax:100, reloadSec:3.0, aiPolicy:null, buyable:true, licensePrice:3200, buildPrice:420},
  {id:'rotary autocannon', name:'14mm Rotary Autocannon', wpnType:'kinetic', fireMode:'spooled-projectile', dmg:5, spd:12, life:120, cd:7, spoolFrames:6, fireSpread:Math.PI*1/180, ammoMax:500, aiPolicy:null, buyable:true, licensePrice:3800, buildPrice:480, drawProjectile:(b)=>{cx.save();cx.fillStyle='#fff';cx.shadowColor='#fff';cx.shadowBlur=sb(5);cx.beginPath();cx.arc(b.x,b.y,2,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'shredder shot', name:'82mm Shredder Shot', wpnType:'kinetic', fireMode:'detonate-projectile', slugDmg:30, slugSpd:6, slugLife:120, pelletCount:8, pelletDmg:8, pelletSpd:15, pelletLife:50, pelletArcRad:Math.PI*25/180, ammoMax:40, cd:0.4, aiPolicy:null, buyable:true, licensePrice:3000, buildPrice:400, drawSlug:(b)=>{cx.save();cx.fillStyle='#bff';cx.shadowColor='#7ff';cx.shadowBlur=sb(12);cx.beginPath();cx.arc(b.x,b.y,6,0,Math.PI*2);cx.fill();cx.globalAlpha=.8;cx.fillStyle='#fff';cx.beginPath();cx.arc(b.x,b.y,2.4,0,Math.PI*2);cx.fill();cx.restore();}, drawPellet:(b)=>{cx.save();cx.fillStyle='#fff';cx.shadowColor='#fff';cx.shadowBlur=sb(4);cx.beginPath();cx.arc(b.x,b.y,1.8,0,Math.PI*2);cx.fill();cx.restore();}, drawProjectile(b){if(b.slug)this.drawSlug(b);else this.drawPellet(b);}},
  {id:'tungsten slugthrower', name:'25mm Tungsten Slugthrower', wpnType:'kinetic', fireMode:'projectile', dmg:50, cd:1.0, spd:16, life:120, ammoMax:40, ricochetsMax:3, aiPolicy:'tap', buyable:true, licensePrice:2400, buildPrice:340, drawProjectile:(b)=>{const sp=Math.hypot(b.vx||0,b.vy||0)||1,tx=-(b.vx||0)/sp*13,ty=-(b.vy||0)/sp*13;cx.save();cx.strokeStyle='#49ffff';cx.globalAlpha=.45;cx.lineWidth=2;cx.beginPath();cx.moveTo(b.x+tx,b.y+ty);cx.lineTo(b.x,b.y);cx.stroke();cx.globalAlpha=1;cx.fillStyle='#dfffff';cx.shadowColor='#49ffff';cx.shadowBlur=sb(10);cx.beginPath();cx.arc(b.x,b.y,2.4,0,Math.PI*2);cx.fill();cx.restore();}},
  {id:'particle accelerator', name:'100 MJ Particle Accelerator', wpnType:'energy', fireMode:'beam',    dmg:100, cd:4.0, range:400, pulses:1, pulseCd:20, persist:4, energyCost:6, chargeDelay:60, beamWidth:6, beamColor:'#8f0', beamSound:[120,.35,'sawtooth',.09], chargeTone:[1200,1800,'sine',.05], aiPolicy:'beam-pulse', buyable:true, licensePrice: 4000, buildPrice: 550},
