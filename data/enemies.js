...
const ENEMY_CLASSES = [
  {
    ...
    sc:300, energy:true, energyMax:30, energyRegenPerSec:4, spinRate:0,
    hull:{angle:'combat', boundsR:19, parts:[{kind:'poly', pts:[[0,-18],[-14,2],[-10,12],[10,12],[14,2]]}]},
    enc:{cnt:1, hp:70, spd:5.0, turn:.05, r:13, col:'#ffaa33', col2:'#ff7700',
      ai:{preferred:150, band:34, strafe:.018,
        pursuit:{lead:.25, offset:85, weave:.7, flipFrames:110, thrust:.052, turnMult:1}},
      fire:{wpn:'mass driver', mode:'aim', count:2, spread:.18, offset:16, minRange:70, arc:.8}},
    drawEnc(e){
  ...
  },
  {
    ...
    sc:280, energy:true, energyMax:40, energyRegenPerSec:5, spinRate:0,
    hull:{angle:'spin', boundsR:15, parts:[{kind:'rect', x:0, y:0, w:26, h:10.4}]},
    enc:{cnt:1, hp:120, spd:3.0, turn:.04, r:13, col:'#aaccff', col2:'#5588dd',
      ai:{preferred:330, band:48, strafe:.01,
        pursuit:{lead:.35, offset:140, weave:.45, flipFrames:170, thrust:.035, turnMult:.9, tangent:.012}},
      fire:{wpn:'railgun', mode:'aim', count:1, spread:0, offset:16, minRange:210, arc:.45}},
    drawEnc(e){
      const ec=this.enc;
  ...
  },
  {
    ...
    enc:{cnt:1, hp:20, spd:7.0, turn:.09, r:9, col:'#ff66cc', col2:'#aa3399',
  ...
  },
  {
    ...
    enc:{cnt:1, hp:20, spd:7.0, turn:.06, r:9, col:'#ffdd33', col2:'#ff8800',
      ai:{passRange:58, commitRange:170, flybyRange:125, resetRange:430, reengageRange:330, minExtendFrames:48, lead:.45, attackSpd:10.4, extendSpd:11.4, turnSpd:7.8, attackThrust:.13, extendThrust:.15, turnThrust:.04, avoidRange:210, passClearance:70, avoidThrust:.04,
        pursuit:{lead:.65, offset:150, weave:.55, flipFrames:140, thrust:.13, turnMult:1, speed:10.4}},
  ...
  },
  {
    ...
    sc:60, energy:false, energyMax:18, energyRegenPerSec:4, spinRate:.1,
    hull:{angle:'spin', boundsR:8, parts:[{kind:'circle', x:0, y:0, r:5}]},
    enc:{cnt:1, hp:10, spd:4.0, turn:.12, r:7, col:'#88ffaa', col2:'#22aa55',
      ...
      fire:{wpn:'mining laser', mode:'aim', count:1, spread:0, offset:8, minRange:10, arc:1.45}},
    drawEnc(e){
      const ec=this.enc;
      cx.save();cx.translate(e.x,e.y);cx.rotate(e.spin);
  ...
  },
  {
    ...
    sc:600, energy:true, energyMax:80, energyRegenPerSec:6, spinRate:0,
    ...
    enc:{cnt:1, hp:480, spd:3.6, turn:.03, r:24, col:'#ddccaa', col2:'#998866',
      ai:{preferred:345, band:64, strafe:.006,
        pursuit:{lead:.2, offset:110, weave:.35, flipFrames:210, thrust:.028, turnMult:.85, tangent:.008}},
      fire:{wpn:'railgun', mode:'aim', count:1, spread:0, offset:28, minRange:0, arc:.85},
...
function assertEnemyRegistry() {
  ...
  for(const e of ENEMY_CLASSES){
    ...
    if(!WEAPON_MAP[e.enc.fire.wpn])throw new Error(`Enemy class ${e.id} references unknown weapon ${e.enc.fire.wpn}`);
    if(e.enc.fire.maxRange!==undefined)throw new Error(`Enemy class ${e.id} must use weapon range from weapons.js, not fire.maxRange`);
  }
